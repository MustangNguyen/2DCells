...
public class PowerUpData_FireBall : PowerUpData
{
    ...
    public override string UpdateDescription(int lv){
        ...
        description = $"Launch fire<color=#00ff00> {fireBallUpgrades[lv].ballsAmount} </color>ball(s) per second to enemies, cause <color=#00ff00>{fireBallUpgrades[lv].damage}%</color> DPS explosive damage.";
        return description;
    }
}
[Serializable]
